...
export async function DELETE(
  ...
  // Next.js 15: params es Promise — await obligatorio antes de acceder a .id
  const { id } = await params;
  const session = await auth();

  console.log('DELETE_DEBUG — ENDPOINT_ID:', id)
  console.log('DELETE_DEBUG — SESSION_USER_ID:', session?.user?.id)
  ...
  try {
    ...
    console.log('DELETE_ROWS_AFFECTED:', dbRes.rowCount)
...
export async function PATCH(
  ...
  // Next.js 15: params es Promise — await obligatorio
  const { id } = await params;
  const session = await auth();
  ...
  try {
    ...
    await pool.query(
      ...
      [name.trim(), url.trim(), method || 'GET', keyword?.trim() || null, id, session.user.id]
    );
